...
const SERVER_HOST = vscode.workspace.getConfiguration('reDocViewer').defaultHost || 'localhost';

const SERVER_PORT = vscode.workspace.getConfiguration('reDocViewer').defaultPort || 9093;
...
export class PreviewServer {
	...
	private startServer(port: number){
		...
		try{
			...
			.listen(this.currentPort).on('error', (err) => {
				if(err) {
					this.startServer(this.currentPort + 1);
				}
			});
